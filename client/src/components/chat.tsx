...
export default function Chat({ userId }: ChatProps) {
  ...
  return (
    <Card className="bg-white/95 backdrop-blur-sm shadow-xl" data-section="chat">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center">
            <MessageCircle className="mr-2 h-5 w-5 text-pink-400" />
            <span className="text-2xl font-bold text-gray-800">Conversa</span>
          </div>
      ...
      <CardContent>
        {/* Chat Messages */}
        <ScrollArea className="h-64 mb-6 chat-messages">
          <div className="space-y-4">
            {conversations.map((conv: any) => (
              <div key={conv.id} className="space-y-2">
                {/* User Message */}
                <div className="flex items-start space-x-3 justify-end">
                  <div className="bg-teal-400 text-white rounded-2xl rounded-tr-sm px-4 py-3 max-w-xs chat-bubble">
                    <p>{conv.message}</p>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="bg-pink-100 rounded-2xl rounded-tl-sm px-4 py-3 max-w-xs chat-bubble">
            ...
            {sendMessageMutation.isPending && (
              <div className="flex items-start space-x-3">
                ...
                <div className="bg-pink-100 rounded-2xl rounded-tl-sm px-4 py-3 max-w-xs">
        ...
        <form onSubmit={handleSubmit} className="flex items-center space-x-3">
          ...
          <div className="flex-1 relative">
            <Input
              ...
              className="pr-12 rounded-full border-2 border-transparent focus:border-pink-400"
